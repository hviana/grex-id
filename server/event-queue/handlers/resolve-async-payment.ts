...
import { updateTenantCache } from "../../utils/instrumentation-cache.ts";
  ...
  if (success) {
    if (isRecurring && sub) {
      ...
      const remainingPlanCredits =
        ((plan?.resourceLimitId as Record<string, unknown>)
          ?.credits as number ?? 0) + creditModifier;
    ...
    if (ctx.systemId && ctx.companyId) {
      updateTenantCache(
        { systemId: ctx.systemId, companyId: ctx.companyId },
        "subscription",
      );
    }
