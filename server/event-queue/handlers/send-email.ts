...
export const sendEmail: HandlerFn = async (payload) => {
  ...
  const senders = payloadSenders && payloadSenders.length > 0
    ? payloadSenders
    : ((await get(undefined, "setting.communication.email.senders")) as
      | string[]
      | undefined) ?? [];
  ...
  const mailgunApiKey = await get(
    undefined,
    "setting.communication.email.mailgun_apikey",
  ) as string | undefined;
  const mailgunUrl = await get(
    undefined,
    "setting.communication.email.mailgun_url",
  ) as string | undefined;
  const mailgunFrom =
    (await get(undefined, "setting.communication.email.mailgun_from") as
      | string
      | undefined) ??
      senders[0];
