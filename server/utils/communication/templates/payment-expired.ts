...
export async function paymentExpiredTemplate(
  locale: string,
  data: {
    name: string;
    systemName: string;
  ...
  const kindSummaryKey = data.kind === "recurring"
    ? "templates.paymentExpired.summaryRecurring"
    : data.kind === "credits"
    ? "templates.paymentExpired.summaryCredits"
    : "templates.paymentExpired.summaryAutoRecharge";

  const kindDetailLabelKey = data.kind === "recurring"
    ? "templates.paymentExpired.planRenewal"
    : data.kind === "credits"
    ? "templates.paymentExpired.creditPurchase"
    : "templates.paymentExpired.autoRechargeLabel";

  const content = `
    <!-- Expired icon with glow -->
    <tr>
      <td style="padding: 0 0 8px 0; text-align: center;">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin: 0 auto;">
          <tr>
            <td style="width: 80px; height: 80px; border-radius: 50%; background: linear-gradient(135deg, rgba(255, 165, 0, 0.18) 0%, rgba(255, 99, 71, 0.12) 100%); text-align: center; vertical-align: middle; border: 1px solid rgba(255, 165, 0, 0.3);">
    ...
    <tr>
      <td style="padding: 16px 0 8px 0; font-size: 22px; font-weight: 700; color: #ffffff; text-align: center; letter-spacing: -0.3px;">
        ${t("templates.paymentExpired.title", locale)}
      </td>
    </tr>
    ...
    <tr>
      <td style="padding: 0 0 24px 0; font-size: 15px; line-height: 1.6; color: #cccccc; text-align: center;">
        ${t("templates.paymentExpired.greeting", locale, { name: data.name })}
      </td>
    </tr>
    ...
    <tr>
      <td style="padding: 0 0 32px 0;">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background-color: rgba(255, 165, 0, 0.06); border: 1px solid rgba(255, 165, 0, 0.15); border-radius: 12px;">
          <tr>
                    ...
                    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                      <tr>
                        <td style="font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; color: #888888; padding: 0 0 4px 0;">
                          ${t("templates.paymentExpired.amountLabel", locale)}
                        </td>
                      </tr>
                      <tr>
                ...
                <tr>
                  <td style="padding: 12px 0;">
                    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                      <tr>
                        <td style="font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; color: #888888; padding: 0 0 4px 0;">
                          ${t("templates.paymentExpired.typeLabel", locale)}
                        </td>
                      </tr>
                      <tr>
                ...
                <tr>
                  <td style="padding: 12px 0 0 0;">
                    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                      <tr>
                        <td style="font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; color: #888888; padding: 0 0 4px 0;">
                          ${t("templates.paymentExpired.systemLabel", locale)}
                        </td>
                      </tr>
                      <tr>
                        <td style="font-size: 15px; font-weight: 600; color: #ffffff;">
    ...
    <tr>
      <td style="padding: 0 0 32px 0; font-size: 14px; line-height: 1.7; color: #aaaaaa; text-align: center;">
        ${t("templates.paymentExpired.body", locale)}
      </td>
    </tr>
    ...
    t("templates.paymentExpired.cta", locale)
  }</span>
  ...
  return {
    title: t("templates.paymentExpired.subject", locale, {
      systemName: data.systemName,
    }),
    body: await emailLayout(
      content,
      locale,
      t("templates.paymentExpired.preheader", locale),
    ),
  };
}
