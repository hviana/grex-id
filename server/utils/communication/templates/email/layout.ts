...
import { get } from "../../../instrumentation-cache.ts";
import type { TenantBanner } from "@/src/contracts/high-level/communication-templates";
...
// TenantBanner is now in @/src/contracts/high-level/communication-templates
...
export async function emailLayout(
  ...
  const appName =
    (await get(undefined, "setting.app.name") as string | undefined) ?? "Core";
  const supportEmail =
    (await get(undefined, "front-setting.front.support.email") as
      | string
      | undefined) ?? "";
