...
export async function listCoreCompanies(
  ...
  >(
    `SELECT id, name, document, createdAt FROM company ${where} ORDER BY createdAt DESC LIMIT $limitPlusOne;
     ...
     LET $total = (SELECT VALUE count() FROM company ${where} GROUP ALL)[0];
     SELECT
       companyId,
       systemId,
  ...
  const companiesRaw = result[0] as
    ...
    | undefined;
  const totalCount = result[2] as number | undefined;
  const companySystems = result[3] ?? [];
  const subs = result[4] ?? [];
  ...
  return {
    items: companies,
    total: totalCount ?? companiesRaw.length,
    hasMore,
