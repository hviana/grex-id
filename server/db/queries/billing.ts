...
// --- validate_voucher (onboarding preview, no subscription needed) -----------
...
export async function validateVoucher(params: {
  ...
  const db = await getDb();
  const [voucherRows] = await db.query<[Record<string, unknown>[]]>(
...
// --- ensure_company_system_tenant (subscribe helper) ------------------------

export async function ensureCompanySystemTenant(params: {
  companyId: string;
  systemId: string;
}): Promise<string> {
  const db = await getDb();
...
export async function subscribe(params: {
  ...
  const userClauses = params.userId
    ? `IF array::len((SELECT id FROM tenant WHERE actorId = $userId AND companyId = $companyId AND !systemId)) = 0 {
       ...
       LET $t1 = (SELECT VALUE id FROM tenant WHERE actorId = $userId AND companyId = $companyId AND !systemId LIMIT 1)[0];
...
export async function removePaymentMethod(
  ...
  const db = await getDb();
  await db.query(
    `LET $pm = (SELECT billingAddressId, tenantIds, isDefault FROM $id);
     DELETE $id;
...
export async function enableAutoRecharge(params: {
  ...
  const pmResult = await db.query<
    [{ id: string }[], unknown[]]
  >(
    `LET $pm = (SELECT id FROM payment_method WHERE tenantIds CONTAINS $tenantId AND isDefault = true LIMIT 1);
     IF array::len($pm) > 0 {
...
export async function retryPayment(
  ...
  >(
    `LET $sub = (SELECT id, retryPaymentInProgress FROM subscription
       WHERE tenantIds CONTAINS $tenantId AND status = "past_due"
       LIMIT 1);
     IF array::len($sub) = 0 {
...
export async function markExpiredPayments(): Promise<ExpiredPaymentRow[]> {
  ...
  const expired = await db.query<[ExpiredPaymentRow[]]>(
    `UPDATE payment SET status = "expired"
     ...
     RETURN id, tenantIds, subscriptionId, kind, amount, currency;`,
  );
  return expired[0] ?? [];
}
...
export async function getPaymentSubscriptionContext(params: {
  ...
  >(
    `SELECT * FROM subscription WHERE id = $id LIMIT 1;
     ...
     IF $voucherId != NONE {
       SELECT resourceLimitId.priceModifier AS priceModifier, resourceLimitId FROM voucher WHERE id = $voucherId LIMIT 1 FETCH resourceLimitId;
     } ELSE {
       SELECT NONE FROM NONE;
     };
...
export async function creditPurchaseOnSuccess(params: {
  ...
  const stmts = [
    `UPSERT usage_record SET
      tenantIds = <set>[$tenantId],
      resourceKey = "credits", value += $amount, counts += 1, period = $period
     WHERE tenantIds CONTAINS $tenantId
...
export async function getAsyncPaymentContext(
  ...
  >(
    `SELECT id, status, subscriptionId, tenantIds, amount, currency, kind FROM payment WHERE id = $id LIMIT 1;
