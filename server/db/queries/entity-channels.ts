...
export async function listChannelsByOwner(
  ...
  const result = await db.query<unknown[]>(
    `LET $owner = (SELECT channelIds FROM ${
      ownerTable(kind)
    } WHERE id = $ownerId)[0];
     IF $owner = NONE THEN [] ELSE
       (SELECT * FROM entity_channel WHERE id IN $owner.channelIds ORDER BY createdAt ASC)
     END`,
    { ownerId: rid(ownerId) },
  );
  const last = result[result.length - 1];
  if (Array.isArray(last)) return last as EntityChannel[];
  if (last && typeof last === "object") return [last as EntityChannel];
  return [];
}
...
export async function findVerifiedOwnerByChannelValue(
  value: string,
): Promise<{ channel: EntityChannel; ownerId: string } | null> {
  const db = await getDb();
  const result = await db.query<unknown[]>(
    `LET $chs = (SELECT * FROM entity_channel
                  WHERE value = $value AND verified = true);
     LET $typedIds = (SELECT VALUE id FROM entity_channel
                       WHERE value = $value AND verified = true);
     IF array::len($chs) > 0 {
       LET $result = (SELECT id, $chs[0] AS channel
                      FROM user WHERE channelIds CONTAINSANY $typedIds LIMIT 1);
       IF array::len($result) > 0 {
         RETURN $result;
       };
     };
     RETURN [];`,
  ...
  const last = result[result.length - 1] as
    | { id: string; channel: EntityChannel }[]
    | undefined;
  const row = last?.[0];
  if (!row) return null;
  return {
    channel: row.channel,
    ownerId: normalizeRecordId(row.id) ?? String(row.id),
  };
}
  ...
  const result = await db.query<unknown[]>(
    `LET $chs = (SELECT * FROM entity_channel
     ...
     LET $typedIds = (SELECT VALUE id FROM entity_channel
                       WHERE type = $type AND value = $value AND verified = true);
     IF array::len($chs) > 0 {
       LET $result = (SELECT id, $chs[0] AS channel, "user" AS ownerKind
                      FROM user WHERE channelIds CONTAINSANY $typedIds LIMIT 1);
       IF array::len($result) > 0 {
         RETURN $result;
       };
       LET $result = (SELECT id, $chs[0] AS channel, "lead" AS ownerKind
                      FROM lead WHERE channelIds CONTAINSANY $typedIds LIMIT 1);
       IF array::len($result) > 0 {
         RETURN $result;
       };
     };
...
/** Counts all verified channels belonging to the owner (any type). */
export async function countVerifiedChannels(
  ownerId: string,
  ownerKind: EntityChannelOwnerKind,
): Promise<number> {
  const db = await getDb();
