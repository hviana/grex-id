...
Migrations live in `server/db/migrations/` (plus
`frameworks/<slug>/server/db/migrations/` and
`frameworks/<name>/server/db/seeds/`). Seeds live in Migrations live in
`server/db/seeds/` (plus `frameworks/<slug>/server/db/seeds/` and
...
node --conditions=react-server server/db/setup.ts
npm run db:setup
...
- **Subsystems** are runtime tenants — one `[slug]` per product. They live in
  `[slug]` subfolders under the relevant Core roots (`systems/<slug>/`) and
  **consume** resources from Core and frameworks.
