...
import { useBearerToken, useTenantContext } from "@/src/hooks/useTenantContext";
import type {
  HumanFace,
  HumanInstance,
  HumanResult,
...
>(
  ({ initialData, companyId, systemSlug, systemToken, botToken }, ref) => {
    ...
    const handleCapture = async () => {
      ...
      try {
        ...
        const token = systemToken ?? bearerToken;
      ...
      <div className="space-y-3">
        ...
        {showBotModal && (
          <Modal
            ...
            title={t("systems.grex-id.face.botRequiredTitle")}
          >
            <p className="text-sm text-[var(--color-light-text)] mb-4">
              {t("systems.grex-id.face.botRequiredMessage")}
            </p>
            <div className="flex justify-end">
              <button
                type="button"
                onClick={() => setShowBotModal(false)}
                className="rounded-lg bg-gradient-to-r from-[var(--color-primary-green)] to-[var(--color-secondary-blue)] px-4 py-2 text-sm font-semibold text-black hover:shadow-lg transition-all"
              >
                {t("common.close")}
              </button>
            </div>
