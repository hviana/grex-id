...
export async function seed(db: Surreal): Promise<void> {
  ...
  const planRolesResult = await db.query<
  ...
  // SurrealDB returns one result per statement; the two IF blocks produce
  // entries at [0] and [1], and the SELECT at [2] has the actual role rows.
  const planRoleIds = [
    ...(planRolesResult[2] ?? []).map((r: { id: string }) => r.id),
    adminRoleId,
  ];
