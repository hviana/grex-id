...
export default function CompaniesPage() {
  ...
  function extractIds(badges: unknown): string[] {
    if (!Array.isArray(badges)) return [];
    return badges.map((b) =>
      typeof b === "string" ? b : (b as { id?: string; name: string }).id ??
  ...
  const fetchCompanies = useCallback(
    async (
      ...
      if (params.filters?.search) {
        sp.set("search", params.filters.search as string);
      }
      if (params.cursor) sp.set("cursor", String(params.cursor));
      sp.set("limit", String(params.limit));
      if (params.filters) {
        const systemIds = extractIds(params.filters.system);
        const planIds = extractIds(params.filters.plan);
        const statuses = extractIds(params.filters.status);
        if (systemIds.length > 0) sp.set("systemIds", systemIds.join(","));
        if (planIds.length > 0) sp.set("planIds", planIds.join(","));
        if (statuses.length > 0) sp.set("statuses", statuses.join(","));
      }
  ...
  const loadChart = useCallback(async () => {
    ...
    const systemIds = extractIds(chartFilters.system);
    const planIds = extractIds(chartFilters.plan);
    const statuses = extractIds(chartFilters.status);
    if (systemIds.length > 0) sp.set("systemIds", systemIds.join(","));
    if (planIds.length > 0) sp.set("planIds", planIds.join(","));
    if (statuses.length > 0) sp.set("statuses", statuses.join(","));
    const res = await fetch(`/api/core/companies?${sp}`, {
    ...
    <div className="space-y-6">
      ...
      <GenericList<CoreCompany>
        ...
        filters={[
          ...
          {
            key: "dateRange",
            label: t("core.companies.dateRange"),
            component: DateRangeFilter,
            props: { maxRangeDays: 31 },
          },
          {
            key: "system",
            label: t("core.companies.systemFilter"),
            component: MultiBadgeFieldFilter,
            props: {
              name: t("core.companies.systemFilter"),
              fetchFn: fetchSystems,
            },
          },
          {
            key: "plan",
            label: t("core.companies.planFilter"),
            component: MultiBadgeFieldFilter,
            props: {
              name: t("core.companies.planFilter"),
              fetchFn: fetchPlans,
            },
          },
          {
            key: "status",
            label: t("core.companies.statusFilter"),
            component: MultiBadgeFieldFilter,
            props: {
              name: t("core.companies.statusFilter"),
