...
export default function MenuTreeEditor(
  ...
  const [formPlanIds, setFormPlanIds] = useState<BadgeValue[]>(
    [],
  );
  ...
  const openEdit = async (item: MenuItemView) => {
    ...
    const planIds = Array.isArray(item.planIds) ? item.planIds.map(String) : [];
    if (planIds.length > 0) {
      try {
        const res = await fetch(
        ...
        setFormPlanIds(planIds.map((id) => ({ id, name: id })));
      }
    } else {
      setFormPlanIds([]);
    }
          ...
          <div key={node.id} className="mt-1.5 first:mt-0">
            ...
            >
              ...
              <div className="flex items-center gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                <EditButton onClick={() => openEdit(node)} />
                <DeleteButton
    ...
    <div className="space-y-3">
      ...
      >
        ...
        <form onSubmit={handleEditSave} className="mt-4 space-y-4">
          ...
          <MultiBadgeField
            name={t("core.menus.planIds")}
            mode="search"
            value={formPlanIds}
            onChange={(vals) => setFormPlanIds(vals)}
            fetchFn={fetchPlans}
          />
