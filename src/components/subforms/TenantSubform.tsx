...
const TenantSubform = forwardRef<SubformRef, TenantSubformProps>(
  (
    ...
    const [roleIds, setRoleIds] = useState<BadgeValue[]>(() => {
      const arr = initialData?.roleIds;
      if (!arr) return [];
      return arr.map((r) => r);
    });
    ...
    const fetchRoles = useCallback(
      ...
      [authHeaders, systemId, systemIdProp, initialGranular],
    );
    ...
    useImperativeHandle(ref, () => ({
      getData: () => {
        ...
        if (show("companyId")) {
          setIfPresent(
            "companyId",
            companyId.length > 0 ? companyId[0].id : "",
          );
        }
        if (show("systemId")) {
          setIfPresent("systemId", systemId.length > 0 ? systemId[0].id : "");
        }
        if (show("systemSlug")) {
          setIfPresent("systemSlug", systemSlug);
        }
        if (show("actorId")) {
          setIfPresent("actorId", actorId.length > 0 ? actorId[0].id : "");
        }
        if (show("actorType")) {
          setIfPresent("actorType", actorType || undefined);
        }
        if (show("roleIds")) {
          setIfPresent(
            "roleIds",
            roleIds.map((r) => (typeof r === "string" ? r : r.id)),
          );
        }
        if (show("groupIds")) {
          setIfPresent(
            "groupIds",
            groupIds.map((g) => typeof g === "string" ? g : g.id),
          );
        }
        if (show("exchangeable")) {
          setIfPresent("exchangeable", exchangeable);
        }
        if (show("frontendUse")) {
          setIfPresent("frontendUse", frontendUse);
        }
        if (show("frontendDomains")) {
          setIfPresent(
            "frontendDomains",
            frontendDomains.map((d) => typeof d === "string" ? d : d.name),
          );
        }
        if (show("isolateSystem")) setIfPresent("isolateSystem", isolateSystem);
        if (show("isolateCompany")) {
          setIfPresent("isolateCompany", isolateCompany);
        }
        if (show("isolateUser")) setIfPresent("isolateUser", isolateUser);
      ...
      isValid: () => {
        ...
        if (
          show("roleIds") && required("roleIds") && roleIds.length === 0
        ) {
          return false;
        }
        if (
          ...
          <div>
            ...
            <SearchableSelectField
              ...
              onChange={(sel) => {
                ...
                onFieldChange?.("systemId", sel.length > 0 ? sel[0].id : "");
              }}
            />
          </div>
        )}
