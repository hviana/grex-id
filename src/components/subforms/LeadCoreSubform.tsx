...
const LeadCoreSubform = forwardRef<SubformRef, LeadCoreSubformProps>(
  ({ initialData, hideTags, companyId, systemId, systemSlug }, ref) => {
    ...
    const [acceptsCommunication, setAcceptsCommunication] = useState(
      initialData?.acceptsCommunication !== false,
    );
      ...
      <div className="space-y-5">
        ...
        <div>
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              type="checkbox"
              checked={acceptsCommunication}
              onChange={(e) => setAcceptsCommunication(e.target.checked)}
            ...
            <span className="text-sm text-[var(--color-light-text)]">
              {t("systems.grex-id.lead.acceptsCommunication")}
            </span>
          </label>
        </div>
