  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        // Load menus + roles (to resolve role IDs → names)
        let tree: {
          componentName?: string;
          children?: { componentName?: string; children?: any[] }[];
        }[] = [];
        try {
          ...
          const customItems = ((menuJson.items ?? []) as RawMenuItem[]).map(
            (item) => ({
          ...
          const visible = flatItems.filter((item) => {
            ...
            // planIds empty = unrestricted. planIds set = only visible when plan is in list.
            if (
              item.planIds?.length &&
              (!plan || !item.planIds.includes(plan.id))
            ) return false;
            return true;
          });
