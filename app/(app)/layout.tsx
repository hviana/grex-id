...
function getDefaultMenus(): MenuItem[] {
  return [
    ...
    },
    {
      ...
      tenantIds: [],
      parentId: undefined,
      name: "common.menu.connectedApps",
      emoji: "🔌",
    ...
    },
    {
      ...
      tenantIds: [],
      parentId: undefined,
      name: "common.menu.connectedServices",
      emoji: "🔗",
...
function buildMenuTree(
  ...
  const visible = flatItems.filter((item) => {
    ...
    // planIds empty = unrestricted (visible in all plans).
    // planIds set = only visible when activePlanId is in the list.
    if (
      item.planIds?.length &&
      (!activePlanId || !item.planIds.includes(activePlanId))
    ) {
      return false;
    }
    return true;
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const loadMenus = useCallback(async (
    ...
    try {
      // Fetch menus and roles in parallel so we can resolve role IDs → names
      ...
      // Build role ID → name map for resolving custom menu roleIds
      const roleIdToName = new Map<string, string>();
      for (const r of rolesJson.items ?? []) {
        if (r.id && r.name) roleIdToName.set(String(r.id), r.name);
      }

      const customItems: MenuItem[] = (menuJson.items ?? []).map(
        (item: MenuItem) => ({
          ...item,
          // Resolve roleIds from record IDs to role names
          roleIds: (item.roleIds ?? []).map((id) =>
            roleIdToName.get(String(id)) ?? String(id)
          ),
        }),
      );
  ...
  useEffect(() => {
    if (authLoading || !systemToken) return;
    if (!ctx.systemId || !ctx.companyId) {
