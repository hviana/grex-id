...
import { getHomePage } from "@/src/frontend-registry";
import { useTenantContext } from "@/src/hooks/useTenantContext";

function HomeContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { t, frontCoreLoaded, publicSystem, publicSystemLoading } =
    useTenantContext();
  const systemSlug = searchParams.get("systemSlug");

  const resolvedSlug = systemSlug ?? publicSystem?.slug ?? null;

  if (!frontCoreLoaded || publicSystemLoading) {
    return (
