...
function mergeCascade(
  ...
  return items.map((item) => {
    const cascade = item._cascade as Record<string, unknown> | undefined;
    if (!cascade) return item;
    const { _cascade, ...rest } = item;
    return { ...rest, ...cascade };
  });
}
...
async function getHandler(req: Request, ctx: RequestContext) {
  ...
  if (action === "search-all") {
    const result = await genericList({
      ...
      cascade: [
        ...
        { table: "resource_limit", sourceField: "resourceLimitId" },
      ],
    ...
    const items = mergeCascade(result.items ?? []);
  ...
  const { companyId, systemId } = ctx.tenantContext.tenant;
  const result = await genericList({
    table: "user",
    select: "id, profileId, channelIds, resourceLimitId, createdAt",
    cascade: [
