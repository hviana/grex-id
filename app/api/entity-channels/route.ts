...
import { rid, setsToArrays } from "@/server/db/connection";
...
async function getHandler(_req: Request, ctx: RequestContext) {
  ...
  return Response.json({ success: true, data: setsToArrays(channels) });
}
...
async function postHandler(req: Request, ctx: RequestContext) {
  ...
  if (action === "resend-verification") {
    ...
    if (
      ...
      ((await genericCount({
          ...
          extraAccessFields: ["id", "channelIds"],
    ...
    const fetchedUser = await genericGetById<{ id: string }>(
      {
        table: "user",
    ...
    const name =
      (fetchedUser?._cascade?.profileId as { name?: string } | null)?.name ??
        "";
  ...
  const maxPerOwner = Number(
    (await get(
      ...
      "setting.auth.entityChannel.maxPerOwner",
    )) || 10,
  );
...
async function deleteHandler(req: Request, ctx: RequestContext) {
  ...
  // Last-verified-channel invariant: removing a verified channel must leave
  // at least one verified channel of any type (§8.7).
  if (channel.verified) {
    const totalVerified = await countVerifiedChannels(userId, "user");
    if (totalVerified <= 1) {
      return Response.json(
        {
          success: false,
          error: {
            code: "VALIDATION",
            errors: ["auth.entityChannel.error.lastVerified"],
          },
        },
        { status: 400 },
      );
