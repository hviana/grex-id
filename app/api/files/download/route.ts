...
export const GET = compose(
  ...
  async (req: Request, ctx: RequestContext): Promise<Response> => {
    ...
    let effectiveTenant: Tenant = ctx.tenantContext?.tenant ?? {};
    ...
    const hitWindowMs = Number(
      (await get(undefined, "setting.cache.file.hitWindowHours")) || "1",
    ) *
      3600000;
    let cacheTenantKey = "core";
    let cacheMaxSize =
      Number((await get(undefined, "setting.cache.core.size")) || "20") *
      1048576;

    const coreData = await get(undefined, "core-data") as any;
    const system = coreData?.systemsBySlug?.[fileSystemSlug];
    ...
    const [dlLimits, bwLimits, defaultConcurrent, defaultBW] = await Promise
      .all([
        ...
        get(undefined, "setting.transfer.default.maxConcurrentDownloads"),
        get(undefined, "setting.transfer.default.maxDownloadBandwidthMB"),
      ]);
