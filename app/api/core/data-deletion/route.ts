...
async function deleteHandler(req: Request, ctx: RequestContext) {
  ...
  const passwordValid = await genericVerify(
    { table: "user", hashField: "passwordHash", skipAccessCheck: true },
  ...
  const system = await genericGetById<{ slug: string }>(
    { table: "system", allowSensitiveGlobalRead: true },
    systemId,
  );
  ...
  const exists = (await genericGetById(
    { table: "company", allowSensitiveGlobalRead: true },
    companyId,
  )) !== null;
  if (!exists) {
    return Response.json(
