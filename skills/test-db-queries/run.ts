...
function printHelp(): void {
  console.log(
    [
      ...
      '  node --conditions=react-server skills/test-db-queries/run.ts -q "SELECT * FROM $id" -b \'{"id":"user:abc"}\'',
      "  node --conditions=react-server skills/test-db-queries/run.ts -f path/to/query.surql",
      "  node --conditions=react-server skills/test-db-queries/run.ts -f query.surql --bindings-file vars.json",
      "  echo 'SELECT * FROM user' | node --conditions=react-server skills/test-db-queries/run.ts --stdin",
      "",
      "Flags:",
