...
3. For each HTTP method the route supports (GET, POST, PUT, DELETE):
   ...
   - Run the request:
     `node --conditions=react-server skills/test-routes/run.ts <METHOD> <PATH>
...
2. Wait for the delivery:
   `node --conditions=react-server skills/test-events/run.ts wait --handler
   <name> --timeout 30000`
...
1. Start the frontend driver:
   `node --conditions=react-server skills/test-frontend/run.ts start`
2. For pages requiring auth, run
   `node --conditions=react-server skills/test-frontend/run.ts login` first.
3. Navigate to the page:
   `node --conditions=react-server skills/test-frontend/run.ts goto <path>`
4. Verify the page renders without errors:
   - `node --conditions=react-server skills/test-frontend/run.ts console --tail 50`
     — check for `pageerror` entries.
   - `node --conditions=react-server skills/test-frontend/run.ts screenshot <page>.png`
     — visual check.
   - `node --conditions=react-server skills/test-frontend/run.ts exists 'main'`
...
The review exercises every test skill, which starts the Next.js dev server. When
you are done, explicitly kill all running Next.js servers:
