...
node --conditions=react-server skills/test-routes/run.ts POST /api/billing \
  --as-superuser \
...
node --conditions=react-server skills/test-events/run.ts wait \
  --handler process_payment \
  --payload-contains '{"kind":"credits"}' \
  --timeout 30000

# 3. Inspect later if needed — or list all recent deliveries for a handler.
node --conditions=react-server skills/test-events/run.ts list --handler process_payment --since 10m
node --conditions=react-server skills/test-events/run.ts stats --minutes 30
```

For dead-letter debugging:

```bash
node --conditions=react-server skills/test-events/run.ts list --status dead --since 1h
...
node --conditions=react-server skills/test-routes/run.ts POST /api/auth/register \
...
node --conditions=react-server skills/test-events/run.ts verification link \
  --action-key auth.action.register \
  --owner-id user:<id>

# 4. Drive the link in a real browser. (Delegates to test-frontend.)
node --conditions=react-server skills/test-events/run.ts verification confirm \
  --action-key auth.action.register \
  --owner-id user:<id>

# 5. Inspect what actually happened across all handlers.
node --conditions=react-server skills/test-events/run.ts stats --minutes 5

# 6. Optional: clean up old rows when iterating.
node --conditions=react-server skills/test-events/run.ts clear --older-than-minutes 15
```
...
node --conditions=react-server skills/test-routes/run.ts POST /api/leads/public \
...
This skill auto-starts the Next.js dev server and keeps it running. When you are
done testing, explicitly kill all running Next.js servers:
