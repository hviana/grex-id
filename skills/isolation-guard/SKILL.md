...
└── NO (anything ambiguous: "add X", "fix Y", "refactor Z", "make a new app")
    └── Run this skill in list-block mode:
        node --conditions=react-server skills/isolation-guard/run.ts
...
```bash
node --conditions=react-server skills/isolation-guard/run.ts --create-subsystem <slug> --name "<Display Name>"

# Example
node --conditions=react-server skills/isolation-guard/run.ts --create-subsystem grex-id --name "Grex ID"
```
...
4. Writes `systems/<slug>/register.ts` (server-side stub — handlers, i18n,
   lifecycle hooks, jobs), `systems/<slug>/src/frontend.ts` (client-side stub —
   components, homepage), and `systems/<slug>/AGENTS.md` (inheriting stub,
   §2.7).
...
7. Adds an import and a call to `register<Slug>()` inside `registerAllSystems()`
   in `systems/index.ts`, and a corresponding import and call to
   `register<Slug>Frontend()` inside `registerAllSystemsFrontend()` in
   `systems/frontend.ts`.
...
```bash
node --conditions=react-server skills/isolation-guard/run.ts --create-framework <name>
```
...
3. Writes `frameworks/<name>/AGENTS.md` (inheriting stub),
   `frameworks/<name>/register.ts` (server-side stub), and
   `frameworks/<name>/src/frontend.ts` (client-side stub).
...
6. Adds an import and a call to `register<Name>Framework()` inside
   `registerAllFrameworks()` in `frameworks/index.ts`, and a corresponding
   import and call to `register<Name>FrameworkFrontend()` inside
   `registerAllFrameworksFrontend()` in `frameworks/frontend.ts`.
...
node --conditions=react-server skills/isolation-guard/run.ts --remove-subsystem <slug>
node --conditions=react-server skills/isolation-guard/run.ts --remove-framework <name>

# Confirmed — deletes everything (exit 0).
node --conditions=react-server skills/isolation-guard/run.ts --remove-subsystem <slug> --yes
node --conditions=react-server skills/isolation-guard/run.ts --remove-framework <name> --yes
```
...
- The import and `register<Slug>Frontend();` call in `systems/frontend.ts`.
...
- The import and `register<Name>FrameworkFrontend();` call in
