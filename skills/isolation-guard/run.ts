...
const CLI = "tsx skills/isolation-guard/run.ts";
...
async function createSubsystem(slug: string, name: string): Promise<void> {
  ...
  try {
    ...
    // 4. INSERT the matching system row so every tenant-scoped route
...
function unwireRegisterFromIndex(params: {
  ...
  const importRegex = new RegExp(
    String
      .raw`^import \{ register as ${alias} \} from "${importSpecifier}";\r?\n?`,
    "gm",
  );
