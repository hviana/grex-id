...
async function cmdLogin(
  ...
  if (!reachable) {
    console.error(
      `[test-routes] server is not reachable at ${baseUrl}. Start it with: node --conditions=react-server skills/test-routes/run.ts server start`,
    );
    return 1;
  }
...
async function cmdRequest(flags: RequestFlags): Promise<number> {
  ...
  try {
    ...
    console.error(
      `[test-routes] is the server running? Try: node --conditions=react-server skills/test-routes/run.ts server status`,
    );
    return 1;
  }
...
function printHelp(): void {
  console.log(
    [
      ...
      "",
      "USAGE",
      "  node --conditions=react-server skills/test-routes/run.ts <command> [args]",
      "",
      "COMMANDS",
      ...
      "EXAMPLES",
      "  # Typical session",
      "  node --conditions=react-server skills/test-routes/run.ts server start",
      "  node --conditions=react-server skills/test-routes/run.ts login",
      "  node --conditions=react-server skills/test-routes/run.ts GET /api/public/front-core",
      "  node --conditions=react-server skills/test-routes/run.ts GET /api/core/systems --as-superuser",
      "  node --conditions=react-server skills/test-routes/run.ts POST /api/core/systems \\",
      '      --body \'{"name":"Foo","slug":"foo"}\' --as-superuser',
      "  node --conditions=react-server skills/test-routes/run.ts server stop",
      "",
      "  # Inline body as a positional argument (convenient for one-liners)",
      '  node --conditions=react-server skills/test-routes/run.ts POST /api/auth/login \'{"identifier":"core@admin.com","password":"core1234"}\'',
      "",
      "  # Form submission with text fields",
      "  node --conditions=react-server skills/test-routes/run.ts POST /api/some-form \\",
      '      --form "name=Alice" --form "email=alice@test.com" --as-superuser',
      "",
      "  # File upload with generated random file",
      "  node --conditions=react-server skills/test-routes/run.ts POST /api/files/upload \\",
