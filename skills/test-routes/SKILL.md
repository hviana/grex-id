...
node --conditions=react-server skills/test-routes/run.ts server logs --tail 80
...
node --conditions=react-server skills/test-db-queries/run.ts 'SELECT id FROM user WHERE roles CONTAINS "admin"'
```
...
response:
`node --conditions=react-server skills/test-routes/run.ts GET /foo 2>/dev/null`.
