...
function assertProjectTsxAvailable(): void {
  // No longer required — the driver is spawned via `node --conditions=react-server`.
}
...
async function sendToDriver(
  ...
  if (!port) {
    throw new Error(
      "driver is not running. Run `node --conditions=react-server skills/test-frontend/run.ts start` first (or any command will auto-start it).",
    );
  }
...
async function ensureDriverRunning(opts: {
  ...
  // forking, so we never confuse the shell's pid with the driver's.
  const cmd =
    `(node --conditions=react-server "${runScript}" __driver > "${logPath}" 2>&1 &)`;
  spawn(cmd, {
    cwd: SKILL_DIR,
    shell: true,
    detached: true,
  ...
  // /health. cold-start + playwright bootstrap can take a while on the
...
function printHelp(): void {
  console.log(
    [
      ...
      "",
      "USAGE",
      "  node --conditions=react-server skills/test-frontend/run.ts <command> [args]",
      "",
      "LIFECYCLE",
      ...
      "  doctor                              (diagnose environment: Playwright, Chromium, state files)",
      ...
      "  node --conditions=react-server skills/test-frontend/run.ts fill 'input[name=\"identifier\"]' core@admin.com",
      "  node --conditions=react-server skills/test-frontend/run.ts fill 'input[name=\"password\"]' core1234",
      "  node --conditions=react-server skills/test-frontend/run.ts click 'button[type=\"submit\"]'",
      "  node --conditions=react-server skills/test-frontend/run.ts wait-for-url '**/entry'",
      "  node --conditions=react-server skills/test-frontend/run.ts screenshot logged-in.png",
      "",
...
async function cmdDoctor(): Promise<number> {
  ...
  checks.push({
    ...
    fix:
      "If stale, run `node --conditions=react-server skills/test-frontend/run.ts stop --all` or " +
