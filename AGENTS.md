...
- **Shared subforms** (`ProfileSubform`, `EntityChannelsSubform`,
  ...
  that collects user/lead communication channels as a list (email, phone, any
  ...
  and public lead submission. Hard-coded email/phone inputs are forbidden for
  list collection. Single-identifier surfaces (login, forgot-password, verify
  resend, account recovery) use a plain text field because they resolve one
  existing verified channel — the user types any value from their verified
  `user.channels`.
...
| `EntityChannelsSubform`        | Entity channels (email/phone/…). **Single authoritative component for collecting user/lead channels** — ProfilePage, lead editor, register, admin user create, public lead submission. Props: `channelTypes: string[]`, `requiredTypes?: string[]`, `mode: "authenticated" \| "local"` (§19.13). `"authenticated"` talks to `/api/entity-channels` for add/verify/resend/delete; `"local"` accumulates channels in memory for unauthenticated or pre-submit forms where the parent request carries them. Single-identifier surfaces (login, forgot-password, verify resend, account recovery) use a plain text input instead, since they resolve one existing verified channel rather than collecting a list. | Profile, Lead editor, Register, Admin user create |
...
**Management UI.** `EntityChannelsSubform` (§18.5) is the single authoritative
UI for collecting entity_channel entries in any form — ProfilePage, lead editor,
register, admin user create, public lead submission. Props:

| Prop            | Type                         | Meaning                                                                                                                                                                                                                          |
| --------------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mode`          | `"authenticated" \| "local"` | `"authenticated"` (default) talks to `/api/entity-channels` for add/verify/resend/delete and renders verification badges. `"local"` keeps channels in memory only; the parent form submits them as part of its own request.      |
| `channelTypes`  | `string[]`                   | Types presented in the add form. Order defines default display.                                                                                                                                                                  |
| `requiredTypes` | `string[]`                   | `"authenticated"`: types the owner must always have at least one **verified** row for (enforced server-side on delete). `"local"`: types the parent form must have at least one entry of before submit; enforced in `isValid()`. |
| `initialData`   | `Record<string, unknown>`    | `local` mode only. Prefill with `{ channels: EntityChannel[] }` to restore an edit-mode list.                                                                                                                                    |
